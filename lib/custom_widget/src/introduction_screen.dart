...
import 'package:ohm_pad/custom_widget/src/ui/intro_button.dart';
import 'package:ohm_pad/custom_widget/src/ui/intro_page.dart';
...
class IntroductionScreenState extends State<IntroductionScreen> {
  ...
  bool _onScroll(ScrollNotification notification) {
    ...
    if (metrics is PageMetrics && metrics.page != null) {
      double overflowPixel = (metrics.viewportDimension * (getPagesLength() - 1)) + 100;
      if(metrics.pixels > overflowPixel) {
        if (!onDoneCalled) {
          print("calling on done ");
          onDoneCalled = true;
          widget.onDone!();
        }
            ...
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    SizedBox(width: MediaQuery.of(Get.context!).size.width/2-30),
                    GestureDetector(
                      onTap: _currentPage >= (totalPages-1) ? widget.onDone : next,
                      child: Stack(
                        ...
                        children: [
                          SemiCircleWidget(
                              diameter: 60,
                              sweepAngle: 360.0.clamp(0.0, 359.90),
                              color: ColorConstants.CAPTION_TEXT_COLOR.withOpacity(0.2)),
                          SemiCircleWidget(
                              diameter: 60,
                              sweepAngle: ((360/totalPages)*(_currentPage+1)).clamp(0.0, 359.90),
